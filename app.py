...
def extract_aso_code(text):
    ...
    # Padrão: M + caracteres + E + 8 dígitos + V + 8 dígitos
...
def check_signature_status(doc, full_text):
    ...
    # 2. Verificação Assinatura Física / Carimbo
    try:
        page = doc[0]
    ...
    if st.button("🚀 Processar, Verificar Assinatura e Renomear"):
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            for idx, uploaded_file in enumerate(uploaded_files):
                ...
                # Organização em pastas dentro do ZIP
                if aso_code and is_valid_sig:
                    zip_path = f"APROVADOS_BENNER/{aso_code}.pdf"
                    final_status = "✅ Pronto para Benner"
                elif aso_code and not is_valid_sig:
                    zip_path = f"REJEITADOS_ASSINATURA/{aso_code}.pdf"
                    final_status = "⚠️ Falha na Assinatura"
                else:
                    zip_path = f"CODIGO_NAO_ENCONTRADO/{original_name}"
        ...
        zip_buffer.seek(0)
        st.download_button(
            label="📦 Baixar Todos os Arquivos Processados (.ZIP)",
            data=zip_buffer,
